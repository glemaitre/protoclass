...
if (count_gt[0] != count_gt[1]):
    ...
    codebook_filename = '/work/le2i/gu5306le/OCT/lbp_r_' + str(radius) + '_hist_now_codebook_random_3_bis/codebook.pkl'
    ...
    def ParallelClassification(idx_test, (pat_test_norm, pat_test_dme), filename_normal, filename_dme, data_folder):
        ...
        for current_cbook in codebook_list[idx_test]:
            ...
            pred_label, roc = Classify(training_data,
                                       ...
                                       n_estimators=100,
                                       n_jobs=70, max_features=None)

            results_by_codebook.append((pred_label, roc))

        return results_by_codebook
        
    results_cv = Parallel(n_jobs=60)(delayed(ParallelClassification)(idx_test, (pat_test_norm, pat_test_dme), filename_normal, filename_dme, data_folder) for idx_test, (pat_test_norm, pat_test_dme) in enumerate(zip(filename_normal, filename_dme)))


    # We have to store the final codebook
    path_to_save = '/work/le2i/gu5306le/OCT/lbp_r_' + str(radius) + '_hist_BoW_now_random_results_3_bis'
