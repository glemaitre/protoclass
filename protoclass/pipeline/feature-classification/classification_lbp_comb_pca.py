...
if (count_gt[0] != count_gt[1]):
    ...
    def get_lbp_data(f):
        ...
        for radius in range(1, 5):
            ...
            filename_data = join(data_folder, f + '_nlm_lbp_' + str(radius) + '_hist.npz')

            # open the current file
            lbp_data.append(np.load(filename_data)['vol_lbp_hist'].reshape(-1))
    ...
    for idx_test, (pat_test_norm, pat_test_dme) in enumerate(zip(filename_normal, filename_dme)):
        ...
        #dmr = KernelPCA(n_components=27, kernel='rbf')
    ...
    path_to_save = '/work/le2i/gu5306le/OCT/lbp_hist_comb_pca_results'
